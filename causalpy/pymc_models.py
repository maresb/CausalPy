...
class StateSpaceTimeSeries(PyMCModel):
    ...
    def build_model(
        ...
        with pm.Model(coords=coordinates) as self.second_model:
            ...
            # FrequencySeasonality already excludes the DC (constant) component by construction,
            # so an additional sum-to-zero constraint on its initial state parameters is not needed
            # for identifiability and can undesirably restrict phase/amplitude (e.g., when n=1).
